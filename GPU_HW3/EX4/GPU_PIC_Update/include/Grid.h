...
#define grd_x(i, j, k) (*(grd->xn + (i) * grd->nyn * grd->nzn + (j) * grd->nzn + (k)))
#define grd_y(i, j, k) (*(grd->yn + (i) * grd->nyn * grd->nzn + (j) * grd->nzn + (k)))
#define grd_z(i, j, k) (*(grd->zn + (i) * grd->nyn * grd->nzn + (j) * grd->nzn + (k)))
#define grd_dim(grd) (grd->nxn * grd->nyn * grd->nzn)

/** Set up the grid quantities */
void setGrid(struct parameters*, struct grid*);
void allocateGridGPU(struct parameters*, struct grid*);
...
void grid_deallocateGPU(struct grid*);

/** interpolation Node to Center */
void interpN2Cfield(FPfield***, FPfield***, FPfield***, struct grid*, struct EMfield*);
