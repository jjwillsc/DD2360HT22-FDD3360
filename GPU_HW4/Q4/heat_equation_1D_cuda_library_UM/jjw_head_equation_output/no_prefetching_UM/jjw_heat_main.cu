...
int main(int argc, char** argv) {
	...
	cusparseDnVecDescr_t vecX, vecY; // Density vectors for SMPV

	bool usePrefetching = false; // Whether use prefetching mode in Unified memory.

	bool flopsCalcMode = false;  // Whether mode to calculate FLOPS - NVIDIA A100 PCIe 40 GB
	float tflops_Of_HPC_GROUP_NJ = 9.746f; // TFLOPS for HPC GROUP NJ NVIDIA - FP64 (double) performance at 9.746 TFLOPS 
	...
	if (argc > 3) {
		flopsCalcMode = atoi(argv[3]) > 0;
	}

	if (argc > 4) {
		usePrefetching = atoi(argv[4]) > 0;
	}
	...
	cputimer_start();
	cudaMallocManaged((void**)&temp, sizeof(double) * dimX, cudaMemAttachGlobal);
	cudaMallocManaged((void**)&tmp, sizeof(double) * dimX, cudaMemAttachGlobal);
	cudaMallocManaged((void**)&A, sizeof(double) * nzv, cudaMemAttachGlobal);
	cudaMallocManaged((void**)&ARowPtr, sizeof(int) * (dimX + 1), cudaMemAttachGlobal);
	cudaMallocManaged((void**)&AColIndx, sizeof(int) * nzv, cudaMemAttachGlobal);
	...
	//Initialize the boundary conditions for the heat equation
	cputimer_start();
	...
	if (usePrefetching || concurrentAccessQ) {
		cputimer_start();
		...
		cudaMemPrefetchAsync(ARowPtr, sizeof(int) * (dimX + 1), device, NULL);
		cudaMemPrefetchAsync(AColIndx, sizeof(int) * nzv, device, NULL);
	...
	cusparseCheck(cusparseCreateCsr(&Adescriptor, dimX, dimX, nzv, ARowPtr, AColIndx, A, CUSPARSE_INDEX_32I,
	...
	cusparseCheck(cusparseSpMV_bufferSize(
		cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, Adescriptor, vecX,
		&zero, vecY, CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, &bufferSize));

	////@@ Insert code to allocate the buffer needed by cuSPARSE
	cudaMallocManaged(&buffer, bufferSize, cudaMemAttachGlobal);
	...
	if (flopsCalcMode) {
		...
		double t = cputimer_stop("SpMV");
		printf("Time spent executing by the GPU: %.8fs\n", t);
		printf("GFLOPs spent executing by the GPU: %.2f\n", t * tflops_Of_HPC_GROUP_NJ * 1000);
	}
	else {
		cputimer_start();
		...
		cputimer_stop("Time step interations");
